...
A **code**  $C$ is a total[^7] mapping between a source alphabet $\Sigma_1$ and the Kleene closure $\Sigma_2^\*$ of another alphabet $\Sigma_2$, called the target or desintation alphabet. 
...
$C$ is called a **viarable-length** code because the number of symbols employed in the target alphabet varies. 
Alternatively
...
In the proceeding, we show (see [Lemmas](#lemmas) and [Appendix](#appendix)) that these three properties uniquely constrain $H$.
...
<!-- Let $L := \| C(X) \|$ the length $C$ is any uniquely decodable code that has minimal expected  -->
Suppose $C : \Sigma_1 \rightarrow \Sigma_2^\*$ is any uniquely decodable code that has minimal expected encoding length, i.e. for $L(X) := \| C(X) \|$, the length of the encoding of $X$, the encoding $C$ has the property that $\mathbb{E}_C[L] \leq \mathbb{E}_C'[L']$
for any other uniquely decodable $C'$ (where $L' := \| C'(X) \|$).
Then 
...
where $\| \Sigma_2 \|$ is the cardinality of $\Sigma_2$.

This means that the lower bound on encoding length (on average) is $\frac{H(X)}{ \log_2 \| \Sigma_2 \|  }$.
...
**Proof**: Let $I := \\{i \,\|\, p_i > 0 \\}$. Then by $\log x \leq x - 1$ for $x > 0$ and $\log x = x - 1$ for $x = 1$ we have 
...
**Theorem**: Let $\Sigma_1 := \\{ s_1, \dots, s_N \\}$ be a source alphabet to be encoded into uniquely decodable code over an alphabet $\Sigma_2$ of size $\| \Sigma_2 \| = r$ with codeword lengths $\\{\ell_1, \dots, \ell_N \\}$.
Then 
...
Conversely, assume $\\{\ell_1, \dots, \ell_N \,\|\, \ell_i \in \mathbb{N} \\}$ satisfy the above inequality.
...
Define $a := \| \Sigma_2 \|$ (cardinality) and $q_i = \frac{a^{-\ell_i}}{R}$ for $i = 1, \dots, N$, with $R := \sum_{i=1}^N a^{-\ell_i}$ (i.e. such that $\sum_{i=1}^N q_i = 1$ and the $q_i$ comprise a discrete probability distribution).
Then
