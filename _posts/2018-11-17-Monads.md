...
Also the explanation will be in the spirit of rigor/correctness rather than formally/technically. So while the code does type check
...
[a Monoid is any type that implements `mappend` and `mempty`, `mappend` combines values of the type and `mempty` 
builds the unit of the type, like 0 for integers]) and arbitrary `a` the type `(w, a)` is a Monad according to the following implementations
of `>>=` and `return`: 
...
in order to produce another state transformer `st'`, and then applies that state transformer `st'` to the as of then current state `s'`.
