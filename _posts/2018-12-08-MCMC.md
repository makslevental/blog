...
$$
E_X\[ f(X) \] \approx \frac{1}{T} \sum\limits_{t=1}^T f\(X_t\)
$$
...
$$
P_{\mbox{trans}}\(X_{t+1}|X_0, X_1, \dots , X_t\)
$$
...
Thus
$$
p_i p_{ij} = \frac{p_i}{r} \min \(1, \frac{p_j}{p_i} \) = \frac{1}{r} \min \(p_i, p_j \) = \frac{p_j}{r} \min \(1, \frac{p_i}{p_j} \) = p_j p_{ji}  
$$
...
Let $p(\textbf{x})$ be the target distribution, where $\textbf{x} = \(x_1, x_2, \dots, x_d \)$. Gibbs sampling consists of a random walk on an undirected graph whose vertices correspond to the values $\textbf{x} = \(x_1, x_2, \dots, x_d \)$ and in which there's an edge from $\textbf{x}$ to $\textbf{y}$ if $\textbf{x}$ and $\textbf{y}$ differ in only one coordinate. Thus, the underlying graph is like a $d$-dimensional lattice except that the vertices in the same coordinate line form a clique.
...
$$
p_{\textbf{x}\textbf{y}} = \frac{1}{d} p\(y_1 |x_2, x_3, \dots, x_d \)
$$

The normalizing constant is $1/d$ since $\sum\limits_{y_1} p\(y_1 \|x_2, x_3, \dots, x_d \) = 1$ and summing over $d$ coordinates 

$$
\sum_\limits{i}^d\sum\limits_{y_i} p\(y_i |x_1, x_2, \dots, x_{i-1},x_{i+1}, \dots, x_d \) =d
$$
...
$$
\begin{eqnarray}
p_{\textbf{y}\textbf{x}} &= \frac{1}{d} p\(x_1 |y_2, y_3, \dots, y_d \) \nonumber\\
&= \frac{1}{d} p\(x_1 |x_2, x_3, \dots, x_d \) \nonumber\\
\end{eqnarray}
$$

where we have used that for $j\neq 1$, $x_j =y_j$. Then we can see that this chain has a stationary distribution proportional to $p\( \textbf{x} \)$

$$
\begin{eqnarray}
p_{\textbf{x}\textbf{y}} &= \frac{1}{d} \frac{p\(x_1 |y_2, y_3, \dots, y_d \)p\(x_2, x_3, \dots, x_d \)}{p\(x_2, x_3, \dots, x_d \)} \nonumber\\
&= \frac{1}{d} \frac{p\(y_1, x_2, x_3, \dots, x_d \)}{p\(x_2, x_3, \dots, x_d \)} \nonumber\\
&= \frac{1}{d} \frac{p\(\textbf{y}\)}{p\(x_2, x_3, \dots, x_d \)} \nonumber\\
\end{eqnarray}
$$
...
$$
\begin{eqnarray}
p_{\textbf{y}\textbf{x}} = \frac{1}{d} \frac{p\(\textbf{x}\)}{p\(x_2, x_3, \dots, x_d \)} 
\end{eqnarray}
$$

from which it follows that $p\(\textbf{x}\) p_{\textbf{x}\textbf{y}}= p\(\textbf{y}\) p_{\textbf{y}\textbf{x}}$ and hence the chain satisfies the detailed balance condition and hence the stationary distribution of the Markov chain is $p_i$ as desired.
